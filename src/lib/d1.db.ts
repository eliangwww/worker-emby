...
import { EmbyPlaybackRecord, EmbySessionRecord } from './emby.types';
...
export class D1Storage implements IStorage {
  ...
  // ---------- Emby 会话 ----------
  async setEmbySession(session: EmbySessionRecord): Promise<void> {
    const db = await this.getDatabase();
    await db
      .prepare(
        `
        INSERT OR REPLACE INTO emby_sessions
        (id, user_id, user_name, access_token, device_id, device_name, client,
         application_version, remote_end_point, created_at, last_activity_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `
      )
      .bind(
  ...
  async getEmbySessionByToken(
    token: string
  ): Promise<EmbySessionRecord | null> {
    try {
      const db = await this.getDatabase();
      const row = await db
        .prepare('SELECT * FROM emby_sessions WHERE access_token = ? LIMIT 1')
        .bind(token)
        .first<any>();
      return row ? this.mapSession(row) : null;
    } catch (err) {
      console.error('Failed to get emby session by token:', err);
      return null;
    }
  }

  async getEmbySessionByDevice(
    userId: string,
    deviceId: string
  ): Promise<EmbySessionRecord | null> {
    try {
      ...
      const row = await db
        .prepare(
          'SELECT * FROM emby_sessions WHERE user_id = ? AND device_id = ? ORDER BY last_activity_date DESC LIMIT 1'
        )
        .bind(userId, deviceId)
        .first<any>();
      return row ? this.mapSession(row) : null;
    } catch (err) {
      console.error('Failed to get emby session by device:', err);
      return null;
    }
  }

  async getAllEmbySessions(): Promise<EmbySessionRecord[]> {
    try {
      const db = await this.getDatabase();
      const result = await db
        .prepare('SELECT * FROM emby_sessions ORDER BY last_activity_date DESC')
        .all<any>();
      return result.results.map((row: any) => this.mapSession(row));
    } catch (err) {
      console.error('Failed to get all emby sessions:', err);
      return [];
    }
  }

  async deleteEmbySession(token: string): Promise<void> {
    const db = await this.getDatabase();
    await db
      .prepare('DELETE FROM emby_sessions WHERE access_token = ?')
      .bind(token)
      .run();
  }

  async touchEmbySession(token: string): Promise<void> {
    try {
      const db = await this.getDatabase();
      await db
        .prepare(
          'UPDATE emby_sessions SET last_activity_date = ? WHERE access_token = ?'
        )
        .bind(Date.now(), token)
        .run();
    } catch (err) {
      console.error('Failed to touch emby session:', err);
    }
  }

  private mapSession(row: any): EmbySessionRecord {
    return {
      Id: row.id,
      UserId: row.user_id,
      ...
      RemoteEndPoint: row.remote_end_point || '',
      CreatedAt: Number(row.created_at) || 0,
      LastActivityDate: Number(row.last_activity_date) || 0,
    };
  }

  // ---------- Emby 播放进度 ----------
  async setEmbyPlayback(record: EmbyPlaybackRecord): Promise<void> {
    const db = await this.getDatabase();
    await db
      .prepare(
        `
        INSERT OR REPLACE INTO emby_playback
        (user_id, item_id, position_ticks, played, play_count, last_played_date, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
      `
      )
      .bind(
        record.UserId,
        record.ItemId,
        Math.max(0, Math.round(record.PositionTicks || 0)),
        record.Played ? 1 : 0,
        record.PlayCount || 0,
        record.LastPlayedDate || Date.now(),
        record.UpdatedAt || Date.now()
      )
      .run();
  }

  async getEmbyPlayback(
    userId: string,
    itemId: string
  ): Promise<EmbyPlaybackRecord | null> {
    try {
      const db = await this.getDatabase();
      const row = await db
        .prepare(
          'SELECT * FROM emby_playback WHERE user_id = ? AND item_id = ? LIMIT 1'
        )
        .bind(userId, itemId)
        .first<any>();
      return row ? this.mapPlayback(row) : null;
    } catch (err) {
      console.error('Failed to get emby playback:', err);
      return null;
    }
  }

  async getAllEmbyPlayback(userId: string): Promise<EmbyPlaybackRecord[]> {
    try {
      const db = await this.getDatabase();
      const result = await db
        .prepare(
          'SELECT * FROM emby_playback WHERE user_id = ? ORDER BY updated_at DESC'
        )
        .bind(userId)
        .all<any>();
      return result.results.map((row: any) => this.mapPlayback(row));
    } catch (err) {
      console.error('Failed to get all emby playback:', err);
      return [];
    }
  }

  async deleteEmbyPlayback(userId: string, itemId: string): Promise<void> {
    const db = await this.getDatabase();
    await db
      .prepare('DELETE FROM emby_playback WHERE user_id = ? AND item_id = ?')
      .bind(userId, itemId)
      .run();
  }

  private mapPlayback(row: any): EmbyPlaybackRecord {
    return {
      UserId: row.user_id,
      ItemId: row.item_id,
      PositionTicks: Number(row.position_ticks) || 0,
      Played: !!row.played,
      PlayCount: Number(row.play_count) || 0,
      LastPlayedDate: Number(row.last_played_date) || 0,
  ...
  async setEmbyFavorite(userId: string, itemId: string): Promise<void> {
    ...
    await db
      .prepare(
        'INSERT OR REPLACE INTO emby_favorites (user_id, item_id, created_at) VALUES (?, ?, ?)'
      )
  ...
  async deleteEmbyFavorite(userId: string, itemId: string): Promise<void> {
    const db = await this.getDatabase();
    await db
      .prepare('DELETE FROM emby_favorites WHERE user_id = ? AND item_id = ?')
      .bind(userId, itemId)
      .run();
  }
  ...
  async getAllEmbyFavorites(userId: string): Promise<string[]> {
    try {
      ...
      const result = await db
        .prepare(
          'SELECT item_id FROM emby_favorites WHERE user_id = ? ORDER BY created_at DESC'
        )
        .bind(userId)
        .all<{ item_id: string }>();
      return result.results.map((row) => row.item_id);
    } catch (err) {
      console.error('Failed to get emby favorites:', err);
      return [];
    }
  }
}
